...
def main():
    ...
    for file in os.listdir(args.rounds_log_dir):
        ...
        if os.path.isdir(d):
            ...
            if (
                os.path.exists(adv_path)
                and os.path.exists(exportstates_path)
                and os.path.exists(exporttrans_path)
                and os.path.exists(exportlabels_path)
            ):
                sb = StrategyBridge(
                    adv_path, exportstates_path, exporttrans_path, exportlabels_path
                )
                ...
                smc.run()
                print(
                    f"SUT value by SMC at {d}: {smc.exec_count_satisfication / smc.num_exec} (satisfication: {smc.exec_count_satisfication}, total: {smc.num_exec})"
                )
                smc_log_path = os.path.join(d, "smc.log")
                with open(smc_log_path, "w+") as f:
                    f.write(
                        f"{smc.exec_count_satisfication / smc.num_exec}\n{smc.exec_count_satisfication}\n{smc.num_exec}"
                    )
